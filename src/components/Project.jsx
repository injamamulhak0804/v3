  ...
  return (
    <>
      <div className="overflow-hidden hidden h-screen md:h-[89%] md:flex 2xl:h-[93%]">
        ...
        <div className="w-[84%] font-fira text-[#607B96] border-e h-[100%] border-[#1E2D3D]">
          ...
          <div className="pt-3 font-fira text-xs border-t border-[#1E2D3D] ">
            <div className="lg:max-w-[90%] flex flex-wrap mx-auto gap-5 w-full  2xl:mx-auto h-[700px] overflow-y-scroll no-scrollbar pb-28">
              {language.toLocaleLowerCase() == "figma" &&
                ProjectDetails.figma.map((item, idx) => (
                  <ProjectCart key={idx} icon={<FaFigma />} data={item} />
                ))}
              {language.toLocaleLowerCase() == "html" &&
                ProjectDetails.html.map((item, idx) => (
                  <ProjectCart key={idx} icon={<FaHtml5 />} data={item} />
                ))}
              {language.toLocaleLowerCase() == "css" &&
                ProjectDetails.css.map((item, idx) => (
                  <ProjectCart key={idx} icon={<FaCss3Alt />} data={item} />
                ))}
              {language.toLocaleLowerCase() == "js" &&
                ProjectDetails.js.map((item, idx) => (
                  <ProjectCart key={idx} icon={<FaJsSquare />} data={item} />
                ))}
      ...
      <div className="py-3 md:hidden bg-[#011627]">
        <div role="tablist" className="tabs tabs-bordered border-b-orange-300">
          ...
          <div
            role="tabpanel"
            className="tab-content border border-b-orange-100 mx-auto py-10"
          >
            {ProjectDetails.react.map((item, idx) => (
              <ProjectCart key={idx} icon={<FaReact />} data={item} />
            ))}
